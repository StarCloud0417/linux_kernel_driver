/**
 * @file   test_cdev_app.c
 * @author Frank Huang
 * @date   2026-03-26
 * @brief  A Linux user space program that communicates with the modern cdev_driver.c LKM
 */
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
...
int main() {
    ...
    printf("Starting cdev device test code example...\n");
    
    // 1. Open the device
    // 確保您已經執行 mknod 建立了 /dev/cdev_device
    fd = open("/dev/cdev_device", O_RDWR);
    if (fd < 0) {
        perror("Failed to open the device /dev/cdev_device (Did you load the module and mknod?)");
        return errno;
    }
